...
class Decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        # 将拼接后的特征传递给Transformer模型
        y_concat = self.transformer(x_concat)
        # 使用torch.split将Transformer输出按照x_length切分成多个部分
        ...
        for idx, tagi in enumerate(self.all_tags[::-1]):
            ...
            # 判断是否在self.trans_tags中
            if tagi in self.trans_tags:
                # 如果在self.trans_tags中，与侧向特征相加
...
class QueryTransformer(nn.Module):
    def __init__(self,
        ...
        # num_gq+num_lq是离散数据的总量，hidden_dim是嵌入向量的维度
        # nn.Embedding就是个大的权重矩阵
