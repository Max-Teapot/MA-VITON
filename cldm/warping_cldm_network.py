...
from cldm.SWSA import SWSA
from cldm.sgattn import SG_Cross_Attention,SG_Self_Attention,FeedForward
...
class StableVITON(UNetModel):
    def __init__(
        ...
        **kwargs,
        ...
        sr_ratios = [8, 8, 8, 4, 4, 4, 2, 2, 2]

        for x_ch, context_ch, sr_ratio in zip(self.encode_output_chs, self.encode_output_chs2, sr_ratios):
            dim_head = x_ch // self.num_heads
            dim_head = dim_head // dim_head_denorm

            warp_flow_blks.append(
                nn.ModuleList([
                SG_Self_Attention(context_dim=context_ch, num_heads=self.num_heads, sr_ratio=sr_ratio),
                SG_Cross_Attention(dim=x_ch, context_dim=context_ch, num_heads=self.num_heads, sr_ratio=sr_ratio),
                FeedForward(dim=x_ch),
                nn.LayerNorm(context_ch),
                nn.LayerNorm(x_ch)
                ]))

            warp_zero_convs.append(self.make_zero_conv(x_ch))
    ...
    def warp(self, x, context, crossattn_layer, zero_conv, mask1=None, mask2=None):
        _, _, H, W = context.shape
        context = rearrange(context, "b c h w -> b (h w) c").contiguous()
        x = rearrange(x, 'b c h w -> b (h w) c').contiguous()
        sg_self_attn = crossattn_layer[0]
        sg_cross_attn = crossattn_layer[1]
        ff = crossattn_layer[2]
        norm1 = crossattn_layer[3]
        norm2 = crossattn_layer[4]
        context, mask = sg_self_attn(context, H, W)
        x = sg_cross_attn(x, norm1(context), H, W, mask) + x
        x = ff(norm2(x)) + x
        x = rearrange(x, 'b (h w) c -> b c h w', h=H, w=W).contiguous()
        output = zero_conv(x) + x
...
class NoZeroConvControlNet(nn.Module):
    ...
    def forward(self, x, hint, timesteps, context, only_mid_control=False, **kwargs):
        ...
        outs = self.SWSA_module(outs, guided_hint)
        return outs, None
